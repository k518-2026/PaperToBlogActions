...
def run_pipeline(dry_run: bool = False, force: bool = False, selected_topic: str = None):
    ...
    if summary is None:
        if dry_run:
            ...
            summary = PaperSummaryModel(
                ...
                point1_what="本研究は、初等・中等教育における<a href=\"https://ja.wikipedia.org/wiki/情報教育\">情報教育</a>およびプログラミング教育を対象に、児童生徒のコンピュテーショナル・シンキングの成長プロセスを可視化・評価することを目的とした包括的研究です。プログラミングの単なる文法習得にとどまらず、論理的思考や抽象化、問題分解といった高次の思考力をいかに授業内で育成できるかについて、理論的背景と教育実践の両面から詳細に検討されています。",
                point2_novelty="従来の研究では、プログラミング教育の成果測定において作成されたプログラムの動作確認やペーパーテストといった最終成果物への依存度が高い傾向にありました。これに対し本論文では、学習者の試行錯誤ログや思考のプロセスデータを多角的に収集・分析する新手法を導入した点が決定的に異なります。<a href=\"https://ja.wikipedia.org/wiki/認知負荷\">認知負荷</a>の理論に基づき、生徒がつまずくポイントをリアルタイムに検知し、指導に活かすフレームワークを世界で初めて体系化した点で先行研究を大きく凌駕しています。",
                point3_core="技術や手法のキモは、学習者のプログラミング作業中に得られる行動ログをもとに、個別最適な足場かけ（スキャフォールディング）を提供する適応型フィードバックアルゴリズムにあります。画一的な正解コードを提示するのではなく、問題解決のヒントを生徒の理解度に合わせて段階的に変化させる対話型プロンプト設計が施されています。これにより、学習者の自律的な思考を奪うことなく、挫折率を最小限に抑える教育的介入を実現しています。",
                ...
                point5_discussion="議論として、教育現場における教員のICTリテラシー格差や、分析ダッシュボードを解釈するための指導者研修の重要性が指摘されています。また、個別最適な学習支援を推進する一方で、協調的な<a href=\"https://ja.wikipedia.org/wiki/アクティブ・ラーニング\">アクティブ・ラーニング</a>とのバランスをどう取るかという教育的配慮の必要性が挙げられており、学校全体の指導計画における位置づけが今後の課題とされています。",
    ...
    # Strictly validate and normalize all Wikipedia links across summary points
